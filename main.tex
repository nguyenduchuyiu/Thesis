...
We report metrics in four groups: proof success, search cost, recovery behavior, and decomposition quality.
...
We also report direct-root solves as a root-only pass@32 baseline. This
baseline gives the model the same local proof budget at the root, but does not allow skeleton expansion
or deeper search. It therefore measures how much of the final solve rate comes from direct local proof
generation before hierarchical search adds any value.
...
We also report the subgoal extraction failure rate for skeleton generations. This measures how often a generated skeleton fails to expose child subgoals that can be inserted into the search graph.
...
Dependency-class distribution is the main structural metric for skeleton quality. Generated child subgoals are classified as solved-and-used, solved-but-unused, unresolved-unused, or unresolved-used. A useful decomposition should create many solved-and-used subgoals, few solved-but-unused subgoals, and few unresolved-used subgoals on active proof paths.

\subsection{Main Results}

Table~\ref{tab:main-results} compares the flat sampling baseline with GammaZero on the evaluated
miniF2F-valid and miniF2F-test rollout logs. The baseline is computed from problems solved directly at
the root by local proof attempts. These are exactly the problems that do not need skeleton expansion or
deeper graph search, so they correspond to the Gemini 3 Flash pass@32 setting used in the rollouts.
...
\midrule
miniF2F-valid & Gemini 3 Flash pass@32 & 13 / 33 & 39.4\% & -- \\
miniF2F-valid & \textbf{GammaZero} & \textbf{27 / 33} & \textbf{81.8\%} & \textbf{+42.4\%} \\
\midrule
miniF2F-test & Gemini 3 Flash pass@32 & 21 / 36 & 58.3\% & -- \\
miniF2F-test & \textbf{GammaZero} & \textbf{28 / 36} & \textbf{77.8\%} & \textbf{+19.4\%} \\
\bottomrule
\end{tabular}
\caption{Comparative evaluation of the flat sampling baseline against GammaZero. Direct root solves are counted as the Gemini 3 Flash pass@32 baseline. Search gain is the absolute improvement in solve rate from hierarchical search.}
...
\bottomrule
\end{tabular}
\caption{Search cost and recovery summary. Actions and Lean calls are averages per theorem; token counts are totals for each split.}
...
On the validation logs, the average syntactic reward \(r_{\mathrm{env}}\) is 0.6717, while the average dependency reward \(r_{\mathrm{dep}}\) is 0.1060. On the test logs, the corresponding values are 0.6781 and 0.0780. The gap between these values is informative. It suggests that many generated fragments can survive recovery or compile with placeholders, but only a smaller fraction contributes to the final proof structure.

This supports the need for dependency-based reward. If the system used only syntactic survival, it would overestimate proof fragments that look well formed but do not help the completed proof. The dependency reward provides a later structural signal: it asks whether the generated declarations are actually used in the proof that Lean checks.

The subgoal extraction failure rate is 45 out of 582 skeleton generations on validation, or 7.73\%.
On test, it is 54 out of 598 skeleton generations, or 9.03\%. This means that most skeleton generations
can be mapped into child subgoals under the scaffold representation. The remaining failures are still
useful diagnostics: they indicate cases where the model produced a skeleton-like output that could not be
safely connected to the focused target hole.

\subsection{Decomposition Analysis}

Table~\ref{tab:deep-metrics} reports the main decomposition and stitching metrics across both splits.
...
Final stitching success & 27 / 27 (100.0\%) & 28 / 28 (100.0\%) \\
...
Subgoal extraction failures & 45 / 582 (7.73\%) & 54 / 598 (9.03\%) \\
...
\bottomrule
\end{tabular}
\caption{Consolidated decomposition and stitching metrics on the miniF2F-valid and miniF2F-test rollout logs. Skeleton counts are reported as total / average added to the search graph per run.}
...
that benefit from intermediate subgoals.

A theorem solved by direct local proof at the root is also the result that would be obtained by a
non-hierarchical root-only baseline using the same local proof proposal budget. In this setup, that number
corresponds to a pass@32-style baseline: the model receives up to 32 local proof attempts at the root,
without skeleton expansion or deeper search. The hierarchical search solves additional depth-greater-than-zero
theorems, showing that skeleton expansion and graph search add coverage beyond direct root completion.

Among solved validation child subgoals, 142 are used in the final proof structure and 9 are solved but
unused. Among solved test child subgoals, 89 are solved and used while only 5 are solved but unused.
Thus, most solved child subgoals contribute to the selected proof path. This supports the skeleton
commitment and priority-scoring design: the search usually spends child-solving effort on subgoals that
remain useful, rather than on branches that disappear from the final proof.

The unresolved classes provide a different diagnostic. The validation logs contain 145 unresolved-unused
and 28 unresolved-used subgoals, while the test logs contain 118 unresolved-unused and 49 unresolved-used
subgoals. Unresolved-unused subgoals mostly correspond to branches that are not part of the selected proof
path or are abandoned before completion. Unresolved-used subgoals are more serious: they occur in proof
structures that still depend on unsolved children. These cases identify bottlenecks where the current local
proof policy or search limits are insufficient.

The dependency distribution also explains why \(r_{\mathrm{dep}}\) is much lower than \(r_{\mathrm{env}}\). Many generated fragments are recoverable, but the final proof uses only a subset of the generated structure. This is expected in a search system: not every explored branch becomes part of the final proof. The important point is that the trajectory record distinguishes useful solved structure from redundant or unfinished structure.

A detailed trajectory case study for \texttt{aime\_1991\_p9} is included in Appendix~\ref{app:case-study-aime-1991-p9}.

\subsection{Ablation Study}

TODO: Insert ablation results after running the comparison settings.

The ablation study compares the full system against variants that remove one design choice at a time. The main comparison settings are:
\begin{itemize}
    \item \textbf{No Sorrifier:} invalid generations are discarded instead of being recovered for partial reward.
    \item \textbf{No scaffolded subgoal solving:} child subgoals are not solved inside their parent scaffold.
    \item \textbf{No local-proof-first rule:} skeleton generation is allowed earlier, without first giving local proof actions priority.
    \item \textbf{No skeleton commitment:} multiple decompositions from the same parent state may compete for search effort.
    \item \textbf{No dependency reward:} skeletons are evaluated without expression-tree dependency analysis.
    \item \textbf{FIFO or level-wise expansion:} open states are expanded in a simpler order instead of using priority-queue selection.
\end{itemize}

Each ablation should use the same theorem set, proposal model, Lean version, generation parameters, and search limits where possible. This keeps the comparison focused on the disabled component rather than on differences in the environment.

TODO: Add the ablation table.

\begin{table}[h]
\centering
\begin{tabular}{lrrrr}
\hline
Setting & Solved & Solve Rate & Avg. Actions & Avg. Lean Calls \\
\hline
Full GammaZero & TODO & TODO & TODO & TODO \\
No Sorrifier & TODO & TODO & TODO & TODO \\
No scaffolded subgoal solving & TODO & TODO & TODO & TODO \\
No local-proof-first rule & TODO & TODO & TODO & TODO \\
No skeleton commitment & TODO & TODO & TODO & TODO \\
No dependency reward & TODO & TODO & TODO & TODO \\
FIFO / level-wise expansion & TODO & TODO & TODO & TODO \\
\hline
\end{tabular}
\caption{Ablation results. Each setting removes one design choice from the full GammaZero system.}
\label{tab:ablation-results}
\end{table}

The ablation analysis should focus on behavior, not only final solve rate. Removing the Sorrifier should show whether recovery provides useful signal or only adds noise. Removing scaffolded subgoal solving should test whether solving children inside the parent scaffold matters. Removing the local-proof-first rule should show whether early decomposition creates unnecessary child states. Removing skeleton commitment should test whether competing decompositions diffuse search effort. Removing dependency reward should show whether syntactic reward alone overestimates redundant skeletons.
...
The selected proof path contains several nested skeleton decompositions. At the root, the successful skeleton introduces the half-angle tangent value as an intermediate fact:
\[
\tan(x/2) = \frac{15}{29}.
\]
...
\]
This part of the trajectory shows the intended role of skeleton actions: they expose intermediate facts that are difficult to discover through flat tactic-level search, while still keeping those facts inside Lean-checkable proof structure.
...
This division of labor is central to GammaZero. Skeleton actions decide which intermediate facts should be introduced. Local proof actions then try to solve the focused child states inside the scaffold where those facts are needed.
