...
\begin{itemize}
    \item \textbf{Model interface:} asks the LLM for local proof actions or skeleton actions under separate output contracts.
    \item \textbf{Lean verification interface:} checks generated candidates, reports diagnostics, returns placeholder states, and rejects candidates that violate the action contract.
...
selects an open state, serializes the scaffold, target goal, and local context, and asks the model for local proof attempts. If
a local proof action closes the state, the state becomes solved and the solution can propagate upward. If local proof attempts fail
but still receive high recoverability scores, the state may receive more local proof attempts. If local closure
...
highest-priority open child, which may itself be solved by a local proof action or decomposed by another skeleton.
...
\subsection{Scaffold Contract in the Implementation}

The scaffold contract from Section~\ref{sec:problem-formulation} is enforced at the level of candidate extraction. When a skeleton creates child subgoals, each child is checked inside a scaffold derived from the parent proof. The focused child is represented by the target \texttt{sorry}, while sibling children are represented by temporary \texttt{admit} placeholders.
...
The model is used as a fixed proposal mechanism. For local proof requests, the prompt asks for Lean proof code that closes the current target and forbids \texttt{sorry} and \texttt{admit}. For skeleton requests, the prompt asks for a proof outline with named child subgoals. \texttt{admit} is forbidden, and \texttt{sorry} is allowed only for named child subgoals, for example:
\begin{verbatim}
have h_step : P := by
  sorry
\end{verbatim}
...
the local-proof-first and skeleton-commitment rules reduce unnecessary decomposition? Does scaffold-aware
...
\begin{itemize}
    ...
    \item local-proof-first expansion and skeleton commitment with reserved fallback states;
    \item execution of local proof and skeleton actions in Lean 4;
...
Generated actions count all local proof and skeleton proposals that pass through the model interface. Lean
...
the decomposition may expose subgoals that are too hard for the current local proof policy.
...
This layered analysis prevents misleading conclusions. A configuration might solve slightly
...
\begin{enumerate}[leftmargin=*]
    ...
    \item Are local proof actions rejected whenever they leave any subgoal?
