...
This gives each state a simple contract: it may replace the target hole with Lean code, but it must leave the surrounding scaffold unchanged.
...
We model search as a graph \(G=(V,E)\). Each OR node is a proof state, and each proof state records the full scaffold and the target position inside that scaffold. From an OR node, the policy selects among alternative terminal tactics or skeletons. A terminal tactic succeeds if it replaces the target \texttt{sorry} with a proof body that Lean accepts in the current scaffold. A skeleton creates an AND node whose children are the generated subgoals.

For an OR node, the value is determined by the best outgoing action:
\[
V(s) = \max_{a \in \mathcal{A}(s)} Q(s,a).
\]
For a skeleton action with children \(S'=\{s'_1,\ldots,s'_k\}\), success requires all children to be solved and stitchable into the parent scaffold. We therefore use a conservative conjunction:
\[
...
For skeleton actions, we use hard AND semantics. A skeleton is marked solved only if all of its generated child proof states are solved and the stitched skeleton passes Lean verification. If a skeleton produces multiple subgoals and even one child remains open when the search limits are reached, the skeleton is not considered solved. Consequently, the skeleton does not receive delayed structural reward or future-value credit from its children. It may still receive immediate syntactic reward from the recovered or successfully compiled skeleton itself. This design discourages the policy from generating large decompositions that solve only easy auxiliary subgoals while leaving difficult branches unresolved.

\subsection{Formal Invariants}

The formulation is governed by a small set of invariants. First, every node with a scaffold has exactly one
target \texttt{sorry}. That target is the only location the node may solve. Second, sibling subgoals are
represented by \texttt{admit} while the current target is being solved, and those \texttt{admit} markers must
remain unchanged by the candidate action. Third, a tactic action can solve the current target but cannot
create child states. Fourth, a skeleton action can create child states by extending the current scaffold
with named proof obligations, but it cannot solve its parent until every required child has been solved
and stitched back into the skeleton. Fifth, a state can be marked \texttt{SOLVED} only by a Lean
verification result for the relevant scaffold position with no \texttt{sorry}, \texttt{admit}, or temporary verification-bypass tactic in the replacement. A
repaired script that contains \texttt{sorry} may contribute reward information, but it cannot solve a node.
These invariants are checked at different points of the pipeline, but together they define the correctness
boundary of the graph.

The final invariant is global: temporary \texttt{admit} placeholders are never part of an accepted theorem.
They are used only to isolate node responsibilities during search. Once the graph claims that a parent
skeleton is solved, GammaZero stitches the solved child proof bodies into the parent scaffold and asks
Lean to verify the assembled block. The root theorem is accepted only after this recursive assembly
reaches the root and Lean verifies the complete theorem without unresolved placeholders. Thus the scaffold
lets the model solve one target at a time while preserving the global proof context and the final Lean
...
Each state with a scaffold records the scaffold hash, target kind, target index, and, when available, the target label inferred from the declaration that contains the target. For example, if the target placeholder appears in \texttt{have h\_step : P := by sorry}, the target label is \texttt{h\_step}. These fields are part of state identity, so two equal-looking goals are not conflated when they occupy different scaffold positions.
